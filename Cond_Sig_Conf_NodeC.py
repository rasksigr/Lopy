...
#        log.close()
        if packet_text == nodeC:
            ...
            while True: #btnA.value == True & btnC.value == True:
                ...
                if btnA.value == False:
                    ...
                    print('"YES" selected')
                    button_a_data = bytes(nodeC_pSig,"utf-8")
                    rfm9x.send(button_a_data)
                    print('pSig should be sent...?')
                    display.fill(0)
                    x=15
                    minX = -6 * len(nodeC_pSig) # 12 = 6 pixels/character * text size 2
                    while x > minX:
                        display.fill(0)
                        display.text(nodeC_pSig, x, 0, 1)
                        display.show()
                        x = x-4
                    display.fill(0)
    ...
    if not btnB.value:
        ...
        button_b_data = bytes(nodeC,"utf-8")
        rfm9x.send(button_b_data)
